...
export const userManagementSchema = yup.object({
  ...
  pin: yup.string().required(PIN_REQUIRED).min(4, PIN_LENGTH_ERROR_MESSAGE).max(4, PIN_LENGTH_ERROR_MESSAGE),
  confirmPassword: yup.string().required(CONFIRM_PASSWORD_REQUIRED).oneOf([yup.ref('password')], PASSWORD_MATCH),
  confirmPin: yup.string().required(CONFIRM_PIN_REQUIRED).oneOf([yup.ref('pin')], PIN_MATCH),
  userLevel: yup.string().required(USERLEVEL_REQUIRED).oneOf(userLevels, USERLEVEL_REQUIRED),
